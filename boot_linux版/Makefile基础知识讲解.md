...
### 7.隐式规则(后缀规则)_解耦在Makefile中的使用

很显然，我们在每一个软件的构建过程中，都强调解耦，那么如果在写Makefile命令的时候，写死的话，肯定是不好的：
#### test1.c:
```c
#include<stdio.h>
int main(){
...
我们又称为强匹配规则，即显式规则。很麻烦。因为即使是相同的逻辑，也要重新写一遍make命令。
那么有什么办法解决？！！————隐式规则应运而生！！
