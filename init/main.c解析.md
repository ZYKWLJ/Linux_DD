...
> 其实这里就是`定一动一`的思想，就和前面的vsprintf的格式符解析哲学思想一样的。

块号毕竟是数字，我们想要查询一个文件时，往往是通过文件名查询(就像域名系统一样)，那么怎么存放块号和文件名的对应关系呢？————通过inode元信息。

#### 2.2inode和inode位图

inode元信息`存放着一个文件的所有信息`，包括`文件大小、创建时间、权限、文件名等等。`

以后，我们每存入一个文件，就得开辟一个inode存放这个文件的元数据。并且，`找到了inode，就获得了该文件的所有信息。`

> 那么显然，有多个文件，也就有多个inode，那我们应该也为inode块整一个位图信息，`理所应当称为inode位图`。

#### 2.3 间接索引
我们的文件有时很大，如果连续存储的话，释放了后，会造成很多内存碎片，不好。
怎么办？使用间接索引。

>即，为一个文件的inode块号记录上对应块号，也存上块目录，这个块目录又指向一个一系列的块。同时这个块目录可以是一级的，也可以是二级的，当然一级就是指向的块，二级指向的是一系列块目录。所以能存更多块。
...
不过，让我们再思考一下，当`inode数量不够用时，我们是怎么得知的`？是不是需要在inode中找，找不到了才知道不够用了？
同样，对于块数量也是如此。

如果我们有一个全局的地方，来记录这些信息就好多了！————`超级块`

超级块中，记录着`文件系统的整体信息`，包括`inode数量、块数量、块大小、空闲块数量`等等。

这是文件系统的上帝视角，我们将至放在最开始的块上，并把他们叫做超级块。如图：

![超级块的视图](image-9.png)


#### 2.5 块描述符
现在，我们块位图、inode位图、inode表固定占据着块1,2,3位置，
假如后面的inode数量很多，使得inode表或者inode位图需要占据多个块，导致其位置会改变，怎么办？

很简单，每个块的起始位置也使用一个块来记录，我们称之为块描述符(`是不是很熟悉？这个我们前面的中断描述符、段描述符是一样的，都是记录的地址等`)

> 块描述符：`记录起始块号、块位图、inode位图、inode表所占块号。`

> 一般来说，块描述符是不变的，因为就是记录起始地址，很简单，永不会超出大小的。

![块描述符的位置](image-10.png)

#### 2.6文件分类

显然，前面都再说文件，没有说目录。我们的目录使用得很多的。如何做到目录的实现呢？
显然，通过我们代表文件一切的信息——`inode块`：

![文件类型(目录+文件)在inode块中体现](image-11.png)

>普通文件的inode指向的数据块就是文件本身，但是目录文件的inode指向数据块中包含了`本目录下的所在文件的inode`，然后这`里面的inode再分别指向各自的文件`。

![文件类型(目录+文件)在inode块中体现](image-12.png)

#### 2.7 根目录
我常说，计算机是死的。
那么根目录也应该定下来，作为文件系统的入口：
我们规定inode表的0号inode，表示根目录，一切的访问，都从这个根目录开始。

![根目录的设置](image-13.png)

现在，我们的文件系统已经很完善了，接下来，让我们欣赏我们的file-system！

#### 2.8 文件系统全貌

![文件系统全貌](image-14.png)

这个和linux上的ext2的经典文件系统基本相同：

![linux上的ext2的经典文件系统](image-15.png)
...
> 这里我们可以看到，回到了`最底层的字节操作`！

##### 1.写字符操作 rw_char
让我们回到之前的read_write.c中的`sys_write操作`的分类主线中来，如果我们是`写字符设备文件`，那么我们就调用`字符设备`的写操作函数rw_char。

> 字符设备，就类似于我们的键盘、鼠标、显示屏等。

块设备和字符设备的区别如下：
| 特性 | **字符设备** | **块设备** |
|------|----------|--------|
| 数据传输单位 | 以**字节/字符流**为单位，**连续传输** | 以`固定大小的块`为单位（如 **512 字节、4KB**），随机访问 |
| 访问方式 | **顺序访问**，**数据传输不可分割（流式）** | 随机访问，支持按块地址读写，可缓存 |
| 典型设备 | `键盘、鼠标、显示器、串口、打印机` | `硬盘、U盘、SD 卡、光驱` |
| 核心特点 | 实时性要求高，数据传输无缓冲或弱缓冲 | 注重存储效率，依赖缓冲区减少物理 IO 次数 |
...
定义了这一串标识符和宏，出处:
`文件模式字（file mode）`
这些宏操作的对象是「文件模式字」（对应struct_inode中的`i_mode`成员，也对应`stat`结构体中的`st_mode`成员），它是一个`32 位整数`（早期为 16 位），按`二进制位划分功能区域`，核心分为两大部分：
```c
//这是struct_inode中的`i_mode`成员
struct m_inode {
	unsigned short i_mode;
//.........
};
//这是`stat`结构体中的`st_mode`成员
struct stat {
...
>用于`标识文件的具体类型`（`普通文件、目录、字符设备`等）；

- 低段位（权限位）：

>分为「`文件所有者 / 组 / 其他用户的读写执行权限`」和「`特殊权限位`」。

具体如下：

```c
文件类型位（高位，bit15 ~ bit12）
特殊权限位（中间位，bit11 ~ bit9）
普通读写执行权限位（低位，bit8 ~ bit0）
```
```c
//全部采用8进制，下面是文件类型掩码！
...
#define S_ISFIFO(m)	(((m) & S_IFMT) == S_IFIFO)//好一个IF和IS，牛逼了
```

| 宏定义   | 八进制值   | 对应功能/文件类型               | 说明                                   |
|----------|------------|--------------------------------|----------------------------------------|
| `S_IFMT` | `00170000` | 文件类型掩码（Type Mask）       | 用于从文件模式字中提取文件类型相关位，过滤无关权限位 |
| `S_IFREG`| `0100000`  | 普通文件（Regular）            | 如文本文件、可执行文件、静态库/动态库文件等       |
| `S_IFBLK`| `0060000`  | 块设备文件（Block Device）      | 如硬盘`/dev/sda`、U盘、SD卡、光驱等               |
| `S_IFDIR`| `0040000`  | 目录文件（Directory）           | 如`/home`、`/etc`、`/tmp`等目录                |
| `S_IFCHR`| `0020000`  | 字符设备文件（Character Device）| 如鼠标、显示器、键盘、串口`/dev/ttyS0`等                 |
| `S_IFIFO`| `0010000`  | 管道/命名管道文件（FIFO）       | 用于进程间通信，实现数据的有序传递              |
| `S_ISUID`| `0004000`  | 设置用户ID（SUID，特殊权限）    | 可执行文件拥有该权限时，普通用户执行会临时获得文件所有者权限（如`/bin/passwd`） |
| `S_ISGID`| `0002000`  | 设置组ID（SGID，特殊权限）      | 1. 可执行文件：执行时临时获得文件所属组权限；2. 目录：目录内新建文件继承目录所属组 |
| `S_ISVTX`| `0001000`  | 粘滞位（Sticky Bit，特殊权限）  | 主要用于公共可写目录（如`/tmp`），仅文件所有者、目录所有者或root可删除/重命名该目录下文件 |

其中，文件类型掩码`S_IFMT`用于从文件模式字中提取文件类型相关位，过滤无关权限位。
因为文件类型位是高段位，所以要和`S_IFMT`进行`按位与操作`，才能得到文件类型位。
而且8进制下的0017等价于`00001111`，用于提取文件类型位。
